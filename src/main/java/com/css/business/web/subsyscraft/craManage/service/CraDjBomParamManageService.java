...
import com.css.business.web.subsyscraft.craManage.dao.CraDjBomParamManageDAO;
...
@Service("craDjBomParamManageService")
public class CraDjBomParamManageService extends
		BaseEntityManageImpl<CraDjBomParam, CraDjBomParamManageDAO> {
	@Resource(name = "craDjBomParamManageDAO")
	// @Autowired
	private CraDjBomParamManageDAO dao;

	@Override
	public CraDjBomParamManageDAO getEntityDaoInf() {
		return dao;
	}
	...
	public void saveOrUpdateBean(List<CraDjBomParam> beans, String account) {
		try {
			for (CraDjBomParam bean : beans) {
				if (bean.getId() == null) {
					// 新建
					...
					// 原BOM 参数
					CraDjBomParam aBean = dao.get(bean.getId());
					// 求原工艺路线
					CraRoute craRoute = crDao.getByCodeAndStep(
							aBean.getProGgxh(),
					...
					if (craRoute == null) {
						...
						craRoute.setProGgxh(bean.getProGgxh());
						craRoute.setSeqName(csDao.getNameByCode(bean
								.getSeqCode()));
						craRoute.setSeqStep(bean.getSeqStep());
						craRoute.setCreateDate(new Date());
						craRoute.setCreateBy(account);
	...
	public CraDjBomParam getByProGgxh(String proGgxh, String seqStep) {
		String hql = "from CraDjBomParam where proGgxh='" + proGgxh
				+ "' and seqStep= '" + seqStep + "'";
		List<CraDjBomParam> listQuery = dao.listQuery(hql);
