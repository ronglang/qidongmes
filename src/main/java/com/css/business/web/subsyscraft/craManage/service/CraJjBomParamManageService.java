...
import com.css.business.web.subsyscraft.craManage.dao.CraJjBomParamManageParamDAO;
...
@Service("craJjBomParamManageService")
public class CraJjBomParamManageService extends
		BaseEntityManageImpl<CraJjBomParam, CraJjBomParamManageParamDAO> {
	@Resource(name = "craJjBomParamManageDAO")
	// @Autowired
	private CraJjBomParamManageParamDAO dao;

	@Override
	public CraJjBomParamManageParamDAO getEntityDaoInf() {
		return dao;
	}
	...
	public void saveOrUpdateBean(List<CraJjBomParam> beans, String account) throws Exception {
		for (CraJjBomParam bean : beans) {
			if (bean.getId() == null) {
				// 新建
				...
				// 原BOM 参数
				CraJjBomParam aBean = dao.get(bean.getId());
				// 求原工艺路线
	...
	public CraJjBomParam getByProGgxh(String proGgxh, String seqStep) {
		String hql = "from CraJjBomParam where proGgxh='" + proGgxh
				+ "' and seqStep= '" + seqStep + "'";
		List<CraJjBomParam> listQuery = dao.listQuery(hql);
	...
	public Page getGridByProGgxh(Page p,String proGgxh){
		String hql = " from CraJjBomParam where 1=1  ";
