...
import com.css.business.web.subsyscraft.craManage.dao.CraJxBomParamManageDAO;
...
@Service("craJxBomParamManageService")
public class CraJxBomParamManageService extends
		BaseEntityManageImpl<CraJxBomParam, CraJxBomParamManageDAO> {
	@Resource(name = "craJxBomParamManageDAO")
	// @Autowired
	private CraJxBomParamManageDAO dao;

	@Override
	public CraJxBomParamManageDAO getEntityDaoInf() {
		return dao;
	}
	...
	public void saveOrUpdateBean(List<CraJxBomParam> beans, String account) throws Exception {
		for (CraJxBomParam bean : beans) {
			if (bean.getId() == null) {
				// 新建
				...
				// 原BOM 参数
				CraJxBomParam aBean = dao.get(bean.getId());
				// 求原工艺路线
	...
	public CraJxBomParam getByProGgxh(String proGgxh, String seqStep) {
		String hql = "from CraJxBomParam where proGgxh='" + proGgxh
				+ "' and seqStep= '" + seqStep + "'";
		List<CraJxBomParam> listQuery = dao.listQuery(hql);
	...
	public Page getGridByProGgxh(Page p,String proGgxh){
		String hql = " from CraJxBomParam where 1=1  ";
