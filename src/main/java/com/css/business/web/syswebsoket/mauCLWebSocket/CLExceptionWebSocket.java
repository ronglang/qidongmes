...
* @Package com.css.business.web.syswebsoket.mauCLWebSocket   
* @Description: 成缆异常的 websocket
* @author   rb
* @date 2017年7月23日 下午6:54:30   
* @company  SMTC
 */
@ServerEndpoint(value = "/CLExceptionWebSocket", configurator = GetHttpSessionConfigurator.class)
public class CLExceptionWebSocket {
	public String text;
		...
		while (session.isOpen()) {
			if (getFlag()) {
				try {
					...
					cacheFactory.removeWebsocketCmdCache(YorkUtil.Memcache_看板_车间_异常_成缆);
				} catch (Exception e) {
	...
	private boolean getFlag() {
		...
		String eId = cacheFactory.getWebsocketCmdCache(YorkUtil.Memcache_看板_车间_异常_成缆);
		if (eId != null && eId!="") {
...
//				text = gson.toJson(mauException);
	...
	@OnClose
	public void onClose() {
		System.out.println("成缆异常的的webSocket  Connection closed");
	}
