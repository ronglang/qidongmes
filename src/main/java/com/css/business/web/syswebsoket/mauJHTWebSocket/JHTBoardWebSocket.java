...
/**     
 * @Title: JHTBoardWebSocket.java   
 * @Package com.css.business.web.syswebsoket.mauJHTWebSocket   
 * @Description: 挤护套异常的websocket  
 * @author   rb
 * @date 2017年7月22日 上午11:12:48   
 * @company  SMTC   
 */
@ServerEndpoint(value = "/JHTBoardWebSocket", configurator = GetHttpSessionConfigurator.class)
public class JHTBoardWebSocket {
	...
	// 刷新条件判断
			...
			while (session.isOpen()) {
				if (getflag()) {
					try {
						...
						throw new Exception("挤护套异常的websocket,出现异常");
					}
				}
				Thread.sleep(6000);
			}
		...
		private boolean getflag() {
			...
			String cache = cacheFactory
					.getWebsocketCmdCache(YorkUtil.Memcache_看板_挤护套_实时);
			if (cache != null && cache != "") {
				...
				cacheFactory
				.removeWebsocketCmdCache(YorkUtil.Memcache_看板_挤护套_实时);
				return true;
			}
			return false;
		}
		...
		@OnClose
		public void onClose() {
			System.out.println("挤护套异常的webSocket  Connection closed");
		}
}
