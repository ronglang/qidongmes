 ...
 * @company  SMTC   
 */
@ServerEndpoint(value = "/JXTaskWebSocket", configurator = GetHttpSessionConfigurator.class)
public class JXTaskWebSocket {
		...
		while (session.isOpen()) {
			if (getFlag()) {
				try {
					...
					throw new Exception("绞线任务进度的websocket,出现异常");
				}
			}
			Thread.sleep(6000);
		}
	...
	private boolean getFlag() {
		...
		String machineId = cacheFactory.getWebsocketCmdCache(YorkUtil.Memcache_看板_绞线_任务);	
		if (machineId!=null && machineId!="") {
			text = machineId;
			cacheFactory.removeWebsocketCmdCache(YorkUtil.Memcache_看板_绞线_任务);
			return true;
		}
		return false;
	}
	...
	@OnClose
	public void onClose() {
		System.out.println("绞线任务进度情况的webSocket  Connection closed");
	}
