 ...
 * @company  SMTC   
 */
@ServerEndpoint(value = "/JYExceptionWebSocket", configurator = GetHttpSessionConfigurator.class)
public class JYExceptionWebSocket {
	public String text;
		...
		while (session.isOpen()) {
			if (getFlag()) {
				try {
					...
					cacheFactory.removeWebsocketCmdCache(YorkUtil.Memcache_看板_车间_异常_绝缘);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					throw new Exception("绝缘异常电子看板八段温度检测表异常的websocket,出现异常");
				}
			}
			Thread.sleep(6000);
		}
	...
	private boolean getFlag() {
		...
		String eId = cacheFactory.getWebsocketCmdCache(YorkUtil.Memcache_看板_车间_异常_绝缘);
		if (eId != null && eId!="") {
