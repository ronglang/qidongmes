...
/**
 * @Title: KZBoardWebSocket.java
 * @Package com.css.business.web.syswebsoket.mauKZWebSocket
 * @Description: 铠装实时数据的websocket
 * @author rb
 * @date 2017年7月22日 上午11:07:28
 * @company SMTC
 */
@ServerEndpoint(value = "/KZBoardWebSocket", configurator = GetHttpSessionConfigurator.class)
public class KZBoardWebSocket {
	// 刷新条件判断
		...
		while (session.isOpen()) {
			if (getflag()) {
				try {
					...
					throw new Exception("绞线部分基础信息异常的websocket,出现异常");
				}
			}
			Thread.sleep(6000);
		}
	...
	private boolean getflag() {
		...
		String cache = cacheFactory
				.getWebsocketCmdCache(YorkUtil.Memcache_看板_铠装_实时);
		if (cache != null && cache != "") {
			...
			cacheFactory
			.removeWebsocketCmdCache(YorkUtil.Memcache_看板_铠装_实时);
			return true;
		}
		return false;
	}
	...
	@OnClose
	public void onClose() {
		System.out.println("铠装工序电子看板基础信息的webSocket  Connection closed");
	}
}
