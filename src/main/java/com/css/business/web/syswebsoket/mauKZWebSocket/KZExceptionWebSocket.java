 ...
 * @Description: 铠装异常异常的webcosket   
 * @author   rb
 * @date 2017年7月23日 下午6:43:09   
 * @company  SMTC   
 */
@ServerEndpoint(value = "/KZExceptionWebSocket", configurator = GetHttpSessionConfigurator.class)
public class KZExceptionWebSocket {
	public String text;
		...
		while (session.isOpen()) {
			if (getFlag()) {
				try {
					...
					cacheFactory.removeWebsocketCmdCache(YorkUtil.Memcache_看板_车间_异常_铠装);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					throw new Exception("铠装异常异常电子看板八段温度检测表异常的websocket,出现异常");
				}
			}
			Thread.sleep(6000);
		}
	...
	private boolean getFlag() {
		...
		String eId = cacheFactory.getWebsocketCmdCache(YorkUtil.Memcache_看板_车间_异常_铠装);
		if (eId != null && eId!="") {
...
//				text = gson.toJson(mauException);
	...
	@OnClose
	public void onClose() {
		System.out.println("铠装异常异常电子看板的webSocket  Connection closed");
	}
