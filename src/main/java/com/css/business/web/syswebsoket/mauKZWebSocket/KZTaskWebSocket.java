...
package com.css.business.web.syswebsoket.mauKZWebSocket;
...
/**     
 * @Title: KZTaskWebSocket.java   
 * @Package com.css.business.web.syswebsoket.mauKZWebSocket   
 * @Description: 铠装任务进度的websocket   
 * @author   rb
 * @date 2017年7月22日 上午11:05:49   
 * @company  SMTC   
 */
@ServerEndpoint(value = "/KZTaskWebSocket", configurator = GetHttpSessionConfigurator.class)
public class KZTaskWebSocket {
		...
		while (session.isOpen()) {
			if (getFlag()) {
				try {
					...
					throw new Exception("铠装任务进度电子看板当前工单任务完成情况异常的websocket,出现异常");
				}
			}
			Thread.sleep(6000);
		}
	...
	private boolean getFlag() {
		...
		String machineId = cacheFactory.getWebsocketCmdCache(YorkUtil.Memcache_看板_铠装_任务);	
		if (machineId!=null && machineId!="") {
			text = machineId;
			cacheFactory.removeWebsocketCmdCache(YorkUtil.Memcache_看板_铠装_任务);
			return true;
		}
		return false;
	}
	...
	@OnClose
	public void onClose() {
		System.out.println("铠装任务进度电子看板当前工单任务完成情况的webSocket  Connection closed");
	}
