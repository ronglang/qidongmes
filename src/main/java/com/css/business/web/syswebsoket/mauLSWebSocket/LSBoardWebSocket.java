...
/**
 * @Title: LSBoradwebSocket.java
 * @Package com.css.business.web.syswebsoket.mauLSdisplayWebSocket
 * @Description: 拉丝电子实时数据的websocket
 * @author rb
 * @date 2017年7月22日 上午10:54:47
 * @company SMTC
 */
@ServerEndpoint(value = "/LSBoardWebSocket", configurator = GetHttpSessionConfigurator.class)
public class LSBoardWebSocket {
	...
	private boolean getflag() {
		...
		String cache = cacheFactory.getWebsocketCmdCache(YorkUtil.Memcache_看板_拉丝_实时);
		if (cache != null && cache != "") {
			...
			String macCode = cacheMap.get("macCode");
			...
			List<Object[]> list = service.getTodayList(macCode, "生产中");
			if (list != null && list.size() > 0) {
				Object[] objs = list.get(0);
				String courseCode = objs[0].toString();
				// 工单材料详情
				PlaMachinePlanMaterManageService plaMachinePlanMaterManageService = SpringContextHolder
						.getBean("plaMachinePlanMaterManageService");
				PlaMachinePlanMater plaMachinePlanMater = plaMachinePlanMaterManageService
						.getPlaMachinePlanMaterByCourseCode(courseCode,machineId);
				if (plaMachinePlanMater != null) {
					...
					cacheMap.put("material_num", plaMachinePlanMater
							.getAmount().toString());
			...
			cacheFactory.removeWebsocketCmdCache(YorkUtil.Memcache_看板_拉丝_实时);
			return true;
		}
		return false;
	}
