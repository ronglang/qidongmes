...
/**     
 * @Title: StoreCrewCominWebSocket.java   
 * @Package com.css.business.web.syswebsoket.storewebsocket   
 * @Description:入库websocket   
 * @author   rb
 * @date 2017年9月27日 上午11:51:10   
 * @company  SMTC   
 */
@ServerEndpoint(value = "/storeCrewCominWebSocket", configurator = GetHttpSessionConfigurator.class)
public class StoreCrewCominWebSocket {
	...
	// 刷新条件判断
		...
		public void OnMessage(String message, Session session, EndpointConfig config)
			...
			while (session.isOpen()) {
				if (getFlag()) {
					try {
						List<StoreObj> list = soService.getTodayList("入库");
		...
		private boolean getFlag() {
			...
			String cache = cacheFactory.getWebsocketCmdCache(YorkUtil.Memcache_看板_仓库_入库);
			if (cache!=null && cache!="") {
				cacheFactory.removeWebsocketCmdCache(YorkUtil.Memcache_看板_仓库_入库);
				return true;
			}
			return false;
		}
