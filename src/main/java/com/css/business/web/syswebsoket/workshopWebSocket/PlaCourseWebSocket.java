 ...
 * @company  SMTC   
 */
@ServerEndpoint(value = "/plaCourseWebSocket", configurator = GetHttpSessionConfigurator.class)
public class PlaCourseWebSocket {
	//刷新条件判断
		...
		private boolean getFlag() {
			...
			String cache = cacheFactory.getWebsocketCmdCache(YorkUtil.Memcache_看板_车间_进度);
			if (cache!=null && cache!="") {
				//有进度更新,查询实时工单完成情况
				//1.查询今日生产的,任务,有哪些工单
				cacheFactory.removeWebsocketCmdCache(YorkUtil.Memcache_看板_车间_进度);
				return true;
			}
			return false;
		}
