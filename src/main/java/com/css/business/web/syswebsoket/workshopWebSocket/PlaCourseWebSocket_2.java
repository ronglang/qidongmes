 ...
 * @company  SMTC   
 */
@ServerEndpoint(value = "/plaCourseWebSocket_2", configurator = GetHttpSessionConfigurator.class)
public class PlaCourseWebSocket_2 {
	//刷新条件判断
		...
		private boolean getFlag() {
			...
			String cache = cacheFactory.getWebsocketCmdCache(YorkUtil.Memcache_看板_生产_饼);
			if (cache!=null && cache!="") {
				//有进度更新,查询实时工单完成情况
				//1.查询今日生产的,任务,有哪些工单
				cacheFactory.removeWebsocketCmdCache(YorkUtil.Memcache_看板_生产_饼);
				return true;
			}
			return false;
		}
