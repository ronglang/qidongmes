...
/**
 * @TODO  : 消息队列 .发送.  消息发完后，需调用close,把连接关闭。
 * @author: 翟春磊
 * @DATE  : 2017年7月14日
 */
public class ApacheMqSender {
	private Logger log = Logger.getLogger(ApacheMqSender.class);
	
	private static ApacheMqSender obj;
	private PropertyFileUtil pfu = new PropertyFileUtil();
	private Connection conn ;
	
	private ApacheMqSender(){
	...
	public void sendMsg_point_to_point(String queue,String msg) throws JMSException{
		if(obj == null) obj = ApacheMqSender.getInstance();
	...
	//用于点对点模式
	public static ApacheMqSender getInstance(){
		if(obj == null){
			obj = new ApacheMqSender();
		}
