...
public class BeanArgumentResolver implements WebArgumentResolver {
	...
	public Object resolveArgument(MethodParameter param,
		...
		RequestBean requestBean = (RequestBean) param
				.getParameterAnnotation(RequestBean.class);
		if (requestBean != null) {
			...
			obj.registerCustomEditor(Date.class, null, new CustomDateEditor(
					new SimpleDateFormat("yyyy-MM-dd"), true));

			System.out.println(obj.findCustomEditor(Date.class, null)
					.getAsText());
			for (String propName : paramsMap.keySet()) {
				String[] propVals = (String[]) paramsMap.get(propName);
				...
				if (props.length == 2) {
					...
					if (tmpObj == null) {
						obj.setPropertyValue(props[1],
								obj.getPropertyType(props[1]).newInstance());
					}
