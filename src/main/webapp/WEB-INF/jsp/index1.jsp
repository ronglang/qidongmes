...
#foot_tab .ft_right .ft_top span{
	...
	function qarWebsocket(){
		var basePath = "<%=basePath%>";
		var urlPirfex = basePath.substring(7, basePath.length);
		var url = "ws://"+urlPirfex+"noticeWebSocket";
	...
	var one_html = "";
			...
			$.post(url,{pcode:id},function(data){
				...
				for(var i =0;i<child.length;i++){
					if(i == 0){
						...
						var h = "";
						if(child[i].name == "车间看板" || child[i].name == "生产部看板"){
							var u = '<%=basePath%>'+child[i].url + "?requestType=portal";
							h ="<li><a href='"+u+"' onclick=getPage('"+child[i].url+"',this,'"+child[i].name+"') target='_blank'> "+child[i].name+"</a></li>";
						}else{
							h="<li><a href='javascript:' onclick=getPage('"+child[i].url+"',this,'"+child[i].name+"')> "+child[i].name+"</a></li>";
						}
						leftMenu.append(h);
					}
				}
		...
		function getPage(url,my,name){
			...
			if(name == "车间看板"){
				$("#mid .m_right iframe").attr('target','_blank');
			}
			var parentsNode = $(my).parent().parent().children();  //li
			for(var i =0;i<parentsNode.length;i++){
				parentsNode.eq(i).children().eq(0).removeClass("selected");
		...
		<div class="m_right" id="m_right" style="display:none">
			<div style="background-color: gray;margin-bottom:40px;">
				...
				<div style="float: left;width:72%;height:30px;">
					...
					<marquee style="float: left;background-color: #FFFFB6;width:95%;height:30px;line-height: 30px;margin-left:10px;color:#CF3327;" id="marq">此处显示消息</marquee>
