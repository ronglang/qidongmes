...
<html>
  <head>
	...
	var basePath = '<%=basePath %>';
		...
		function ajaxData(){
			...
			$.ajax({
				url: basePath+"rest/mauMachineFaultManageAction/getFaultList",
				dataType: 'json',
				type: 'POST',
		...
		function fault(list){
			...
			for(var i=0;i<list.length;i++){
				if(i%2 == 0){
					html+= "<li class='rs'><span>"+(i+1)+"</span><span>"+list[i].macCode+"</span><span>"+list[i].faultDate+"</span><span>"+list[i].faultType+"</span><span>"+list[i].status+"</span></li>";
				}else{
					html+= "<li class='ls'><span>"+(i+1)+"</span><span>"+list[i].macCode+"</span><span>"+list[i].faultDate+"</span><span>"+list[i].faultType+"</span><span>"+list[i].status+"</span></li>";
				}
  ...
  <div class="ml_con">
		...
		<div class="title" style="margin-top: 10px;">
			<span>序号</span><span>设备编码</span><span>故障时间</span><span>故障类型</span><span>状态</span>
		</div>
		<div id="mlc_list_t">
