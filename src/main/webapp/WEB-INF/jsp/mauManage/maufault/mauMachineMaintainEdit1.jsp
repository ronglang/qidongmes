	...
	<input type="hidden" name="id" id="hideId"  value="${id}"/>
		...
		<table cellpadding="0" cellspacing="0" class="l-table-edit">
			<tr class="l-table-edit-tr">
				...
				<td align="left" class="l-table-edit-td-input"><input id="remark"  name="remark" type="text" ltype="text"  value="${mauMachineMaintain.remark }"/></td>
			...
			<td align="right" class="l-table-edit-td">进行状态:</td>
				...
				<select  id="status">
					<option >--请选择--</option>
					<option value="1" <c:if test="${mauMachineMaintain.status=='进行中'}">selected</c:if>>进行中</option>
					<option value="2" <c:if test="${mauMachineMaintain.status=='已完成'}">selected</c:if>>已完成</option>
				</select>
				</td>	
			</tr>
		</table>
