...
<html>
  <head>
	...
	var urlPirfex = basePath.substring(7, basePath.length);
		...
		function initData(){
			$.ajax({
				url: basePath + "rest/storeObjManageAction/getTodayObj?inoutType=入库",
				type: 'post',
				dataType : 'json',
		...
		function instore(list){
			...
			for(var i=0;i<list.length;i++){
				if(i%2 == 0){
					html+= "<li><span class='s1'>"+list[i].objSort+"</span><span class='s2'>"+list[i].objGgxh+"</span><span class='s1'>"+list[i].acount+"</span><span class='s1'>"+list[i].unit+"</span><span class='s2'>"+list[i].inDate+"</span><span class='s1 s3'>"+list[i].materialType+"</span></li>";
				}else{
					html+= "<li class='ls'><span class='s1'>"+list[i].objSort+"</span><span class='s2'>"+list[i].objGgxh+"</span><span class='s1'>"+list[i].acount+"</span><span class='s1'>"+list[i].unit+"</span><span class='s2'>"+list[i].inDate+"</span><span class='s1 s3'>"+list[i].materialType+"</span></li>";
				}
				
			}
			html+="</ul>";
			html+="</div>";
			$("#mlc_list_t").append(html);
			//通知滚动
			$("#mlc_list_t .mlcl_ul").kxbdSuperMarquee({
				isMarquee:true,
				isEqual:false,
				scrollDelay:200,
				direction:'top'
		...
		function myWebsockt(){
			...
			var url = "ws://"+urlPirfex+"storeCrewCominWebSocket";
  ...
  <body>
    ...
    <div class="ml_con">
		...
		<div class="title" style="margin-top: 10px;">
			...
			<span calss="t1" style="width: 80px;">入库数量</span>
			<span calss="t2" style="width: 80px;">单位</span>
			<span calss="t2" style="width: 160px;">入库时间</span>
