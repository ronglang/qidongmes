var tbdArray = new Array();
var grid;
$(function() {
	// pub_initList(routeName);
	grid = $("#sysContactGroupManageList")
			.ligerGrid(
					{
						url : basePath + "rest/" + routeName
								+ "Action/vwdataGridPage",
						checkbox : true,
						columns : [
								{
									display : '序号',
									name : 'id',
									hide:true,
									width : 1
								},
								{
									display : '组名',
									name : 'name',
									width : 180
								},
								{
									display : '维护人',
									name : 'createBys',
									width : 120
								},
								{
									display : '维护时间',
									name : 'createDate',
									width : 220
								},
								/*{
									display : '更新人',
									name : 'updateBy',
									width : 120
								},
								{
									display : '更新时间',
									name : 'updateDate',
									width : 120
								},
								{
									display : '备注',
									name : 'remark',
									width : 220
								},*/
								{
									display : '操作',
									isAllowHide : false,
									width : 120,
									render : function(row) {
										var html = '<a href="#" onclick="javascript:edit('
												+ row.id
												+ ');return false;">编辑</a>&nbsp;&nbsp;&nbsp;';
										html += '<a href="#" onclick="javascript:show('
												+ row.id
												+ ');return false;">查看</a>';
										return html;
									}
								} ],
						pageSize : 10,
						isChecked : f_isChecked,
						onCheckRow : f_onCheckRow,
						onCheckAllRow : f_onCheckAllRow,
						width : '100%',
						height : '97%',
						onSuccess : function(json, grid) {
							$("#pageloading").hide();
						},
						onError : function(json, grid) {
							$("#pageloading").hide();
						}
					});
	$("#add").click(function() {
		top.$.ligerDialog.open({
			url : basePath + "rest/sysContactGroupManageAction/toAddEditPage",
			height : 600,
			width : 750,
			modal : true,
			title : '添加分组'
		});
	});
	$("#all").click(function() {
		var pa = $("#" + routeName + "ListForm").serialize();
		grid.loadServerData(decodeURIComponent(pa, true));
	});
});

function edit(id) {
	top.$.ligerDialog.open({
		url : basePath + "rest/sysContactGroupManageAction/toAddEditPage?id="
				+ id,
		height : 600,
		width : 750,
		modal : true,
		title : '编辑分组'
	/*
	 * buttons: [ { text: '确定', onclick: function (item, dialog) {
	 * alert(item.text); } },
	 * 
	 * {text: '取消', onclick: function (item, dialog) { dialog.close(); } } ]
	 */});
}

function reloadData() {
	var pa = $("#" + routeName + "ListForm").serialize();
	grid.loadServerData(decodeURIComponent(pa, true));
}

function show(id) {
	top.$.ligerDialog.open({
		url : basePath + "rest/sysContactGroupManageAction/toDetailPage?id="
				+ id,
		height : 600,
		width : 750,
		modal : true,
		title : '查看分组'
	/*buttons: [
	{text: '取消', onclick: function (item, dialog) { 
		dialog.close(); 
	} } 
	]*/});
}
