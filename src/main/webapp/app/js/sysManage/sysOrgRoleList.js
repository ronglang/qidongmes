...
$(function ()
    {
		...
		grid=$("#sysOrgRoleManageList").ligerGrid({
			url : basePath + "rest/"+ routeName + "Action/dataGridPage",
	        checkbox: true,
	        columns: [
	            { display: '序号', name: 'id',  width: 120 },{ display: '', name: 'orgId',  width: 120 },{ display: '', name: 'roleId',  width: 120 },
		        {
		...
		$("#add").click(function(){
			$.ligerDialog.open({ 
				url: basePath+"rest/sysOrgRoleManageAction/toAddEditPage", 
				height: 600,
				width: 1000,
				modal:true, 
...
function edit(id){
	$.ligerDialog.open({ 
		url: basePath+"rest/sysOrgRoleManageAction/toAddEditPage?id="+id, 
		height: 600,
		width: 900,
		modal:true, 
		/*buttons: [ { text: '确定', onclick: function (item, dialog) { 
...
function show(id){
	$.ligerDialog.open({ 
		url: basePath+"rest/sysOrgRoleManageAction/toDetailPage?id="+id, 
		height: 600,
		width: 900,
		modal:true, 
		/*buttons: [
